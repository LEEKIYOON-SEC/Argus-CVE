...
def generate_report_content(cve_data, reason):
    ...
    2. Format: Markdown
    3. Structure:
       - 🛑 **요약 (Summary)**: 2-3 sentences explaining the vulnerability simply.
       - 🔍 **기술적 세부사항 (Technical Details)**: What is the attack vector?
       - 🛡️ **대응 방안 (Mitigation)**: General advice based on the description.
    ...
    try:
        ...
        chat_completion = client.chat.completions.create(
            ...
            model=GROQ_MODEL_ID, # 여기에 지정된 모델 ID 사용
            temperature=0.3,     # 보안 분석이므로 창의성보다는 정확성(0에 가깝게)
        )
        ai_analysis = chat_completion.choices[0].message.content
    except Exception as e:
        print(f"[WARN] Groq AI Failed ({GROQ_MODEL_ID}): {e}")
        # 실패 시 원문 설명으로 대체 (Fallback)
        ai_analysis = f"⚠️ **AI 분석 실패 (모델 오류)**\n\n원문 설명:\n{cve_data['description']}"

    # 최종 리포트 조립
    return f"""
# 🛡️ Argus Threat Intelligence Report
**Target:** {cve_data['id']}
...
- **EPSS Score:** {cve_data['epss']*100:.2f}% (실제 공격 확률)
- **KEV(실제 악용):** {'🚨 YES (CISA 경고)' if cve_data['is_kev'] else 'No'}

---

## 🤖 AI Security Analysis (Korean)
**Model:** `{GROQ_MODEL_ID}`
