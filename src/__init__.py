...
def fetch_official_rules(cfg, cve_id: str) -> List[Dict[str, Any]]:
    """
    Official/Public rules aggregator (best-effort).
    This symbol MUST exist at package top-level because src.main imports it as:
    ...
    def _dedup(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        out = []
        for h in items:
            h["engine"] = _normalize_engine(h.get("engine"))
            key = (
                h.get("engine"),
